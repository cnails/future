...
class Files(QtWidgets.QMainWindow, tree_gui.Ui_MainWindow):
    ...
    def accept_append(self):
        # lis = []
        data =  {'accept_append': True, 'chat_id': self.parent.chat_id}
        for i, user in enumerate(self.parent.users):
            if i == 0:
                continue
            if globals()[f'self.check_{i}'].isChecked():
                # lis.append(user)
                data[user] = True
    ...
    def __init__(self, parent):
        ...
        # print(self.parent.users)
        self.owner = False
        list_accepted = req.get(url, data = {'return_accept': self.parent.chat_id}).json()
        ...
        for i, user in enumerate(self.parent.users):
            ...
            globals()[f'self.check_{i}'] = QtWidgets.QCheckBox(self.formLayoutWidget)
            if user in list_accepted:
                globals()[f'self.check_{i}'].setChecked(True)    
            globals()[f'self.check_{i}'].setText("")
            globals()[f'self.check_{i}'].setObjectName(f"check_{i}")
        ...
        self.push_file_button_3.clicked.connect(self.accept_append)
    ...
    def check_login(self):
        ...
        if len(self.login) < 3:
            ...
            if response.json():
                ...
                self.hide()
                window = ExampleApp(self.login, chats=True)
                # window.show()
            else:
                try:
                    self.short_login.hide()
